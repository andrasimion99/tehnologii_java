...
public class GenericDaoImpl<T> implements GenericDao<T> {
    ...
    @Override
    public List<T> getAllByProperty(String query, String prop) {
        return em.createNamedQuery(query).setParameter(1, prop).getResultList();
    }

    @Override
    public List<T> getAllByProperty(String query, int prop) {
        return em.createNamedQuery(query).setParameter(1, prop).getResultList();
    }
